...
def plot_optimization_results(preferences, initials, finals):
    ...
    labels = ['Highly\nUndesirable', 'Undesirable', 'Tolerable',
                               'Desirable', 'Highly\ndesirable']
    fig, ax = plt.subplots()
    xticks = []
    y_vals = np.arange(len(preferences)) + 0.5
    x_max = max(y_vals)  # to keep squareish
    for i, (initial, final, (prefname, pref))  in enumerate(zip(initials, finals, preferences.items())):
        ...
        ax.annotate('', xy=(final_f, y), xytext=(initial_f, y), arrowprops={'arrowstyle':'->'})
    ...
    plt.yticks(list(reversed(y_vals)), [s.capitalize() for s in preferences.keys()])
